...
def rollingavgWD(data):
    for site in data['SiteCode'].unique():
        ...
        newdata = pd.merge(da2018[['counter','2018']], da2019[['2019','counter']], how='outer', on=['counter']).reset_index(drop=True)
        newdata = pd.merge(newdata, da2020[['2020','counter','date']], how='outer', on=['counter']).reset_index(drop=True)
        ...
        filestring = 'rollingavgWD_'+thisdata['TrafficType'].iloc[0] + '_' + thisdata['SiteName'].iloc[0].replace(' ', '').replace('/','')
        ...
        #uploadaverages(site,thisdata,newdata, filestring, namestring, folder, chartadminfn, updatedate)
